...
if [ -z "$1" ]; then
  echo "usage download_and_preprocess_coco.sh [output dir] [raw data dir]"
  exit
fi
...
function create_train_dataset() {
  ...
  TRAIN_OBJ_ANNOTATIONS_FILE="${SCRATCH_DIR}/annotations/instances_attributes_train2020.json"
...
function create_val_dataset() {
  ...
  VAL_IMAGE_DIR="${SCRATCH_DIR}/test"
  VAL_OBJ_ANNOTATIONS_FILE="${SCRATCH_DIR}/annotations/instances_attributes_val2020.json"
...
function create_testdev_and_test_dataset() {
  ...
  TEST_IMAGE_INFO_FILE="${SCRATCH_DIR}/annotations/info_test2020.json"
  PYTHONPATH="tf-models:tf-models/research" python3 $SCRIPT_DIR/create_coco_tf_record.py \
    --logtostderr \
    --include_masks \
